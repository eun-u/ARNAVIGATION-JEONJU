...
data class JeonjuState(
    ...
    val frames: Int=0,val reroutes: Int=0,val decision: String="대기",val calibration: MapCalibration?=null,
...
class JeonjuCoordinator(private val context: Context,val mode: String,val clipId: String,val backend: String,private val autoStart: Boolean=false,requestedRunId: String?=null,accessToken: String="") : AutoCloseable {
    ...
    val state=MutableStateFlow(JeonjuState())
    ...
    private suspend fun prepare() {
        try {
            pocDeadline("preparation",35_000) {
                val b=JSONObject(api.getJeonjuBootstrap().toString())
                ...
                state.update{it.copy(prepared=true,message=if(mode=="Replay")"재생 입력 검증 완료 · 시작을 누르세요." else "두 기준점에서 정합을 준비한 뒤 시작하세요.")}
            }
            if(mode=="SelfTest") {
    ...
    fun start() {
        ...
        job=scope.launch {
            try {
                ...
                require(current.capture?.depth!=null && current.capture?.semantics!=null){"Depth·Scene Semantics 입력을 확인하세요."}
                require(current.trackingQuality==TrackingQuality.TRACKING && (current.accuracy?.horizontalMeters ?: 99.0)<=1.5){"현재 위치 정합 오차가 큽니다."}
                val token=++generation
                val origin=requireNotNull(current.geoCoordinate)
                val route=pocDeadline("initial_route",12_000){api.startJeonjuRoute(RouteRequestDto(CoordinateDto(origin.latitude,origin.longitude),b.getJSONObject("destination").dto(),"demo_jeonju"))}
                ...
                active=true
                state.update{it.copy(running=true,starting=false,calibration=calibration,message="자동 안내 중 · 접근성 미확인 시연 경로")}
                ...
                if(mode=="Live") pipeline=LatestFramePipeline(scope,engine,::processFrame){error->scope.launch{fail("프레임 처리 실패",error)}}
                else {
                    withContext(Dispatchers.Default) {
                        engine.use {
    ...
    private suspend fun processFrame(s: SpatialFrameContext,frame: PerceptionFrameLease,result: PerceptionResult) {
        ...
        val observations=fusion.fuse(s,result)
        val before=state.value
        val progress=before.snapshot?.let{progressTracker.update(it,s)}
        val valid=s.trackingQuality==TrackingQuality.TRACKING && s.capture?.calibration!=null && (s.accuracy?.horizontalMeters ?: 99.0)<=1.5 && progress?.remainingMeters!=null
        val recovered=before.arGuidanceDeferred && valid && rerouteAttempts.cooldownElapsed(System.nanoTime()) &&
            PocReroutePolicy.positionReason(scopeSegments,s,s.capture?.calibration?.revision,if(mode=="Live")System.currentTimeMillis() else null)==null
        val arDeferred=before.arGuidanceDeferred && !recovered
        withContext(Dispatchers.Main) {
            if(!active)return@withContext
            state.update{it.copy(frames=it.frames+1,decision=fusion.lastReason,spatial=s,detections=result.detections,rotation=frame.rotationDegrees,
        ...
        event("frame",JSONObject().put("frame_id",s.stamp.frameId).put("timestamp_ns",s.stamp.timestampNanos).put("observed_at_epoch_ms",s.capture?.observedAtEpochMillis)
            .put("inference_ms",result.inferenceMillis).put("detections",result.detections.size).put("decision",fusion.lastReason)
        ...
        if(mode=="Live" && (latestSpatial?.trackingQuality!=TrackingQuality.TRACKING || latestSpatial?.capture?.calibration?.revision!=s.capture?.calibration?.revision))return
        for(observation in observations) {
            ...
            event("observation",JSONObject().put("frame_id",s.stamp.frameId).put("observation_id",observation.observationId).put("edge_id",impact?.edgeId ?: JSONObject.NULL).put("decision",if(impact==null)"ambiguous_or_outside_route" else "temporary_avoidance")
                ...
                .put("object_position",observation.geoCoordinate?.let(::geoJson)).put("accuracy_m",observation.horizontalAccuracyMeters))
    ...
    private suspend fun reroute(s: SpatialFrameContext,impact: EdgeImpact,snapshot: RouteSnapshot) {
        ...
        val current=if(mode=="Live")requireNotNull(latestSpatial) else s
        val token=generation;val attemptKey="${impact.edgeId}:${o.observationId}"
        if(!active)return
        val localReason=PocReroutePolicy.positionReason(scopeSegments,current,o.calibrationRevision,if(mode=="Live")System.currentTimeMillis() else null)
        ...
        p.put("current_position",geoJson(user).put("accuracy_m",current.accuracy?.horizontalMeters).put("timestamp",iso(if(mode=="Live")requireNotNull(current.capture).observedAtEpochMillis else System.currentTimeMillis())).put("calibration_revision",o.calibrationRevision))
        val evidence=JSONObject().put("object_position",geoJson(requireNotNull(o.geoCoordinate))).put("accuracy_m",o.horizontalAccuracyMeters)
        ...
        p.put("avoidance_upserts",JSONArray().put(JSONObject().put("edge_id",impact.edgeId).put("observation_id",o.observationId).put("frame_id",s.stamp.frameId.toString())
            .put("observed_at",iso(requireNotNull(s.capture).observedAtEpochMillis)).put("source",if(mode=="Replay")"replay_ai" else "live_ai").put("ttl_seconds",60).put("evidence",evidence)))
        ...
        try {
            ...
            event("route_applied",r.put("frame_id",s.stamp.frameId).put("observation_id",o.observationId).put("edge_id",impact.edgeId)
                ...
                .put("reaction_ms",if(mode=="Live")System.currentTimeMillis()-(requireNotNull(s.capture).observedAtEpochMillis-o.persistenceMillis) else (System.nanoTime()-(replayStartedNanos+s.stamp.timestampNanos-o.persistenceMillis*1_000_000-replayFirstTimestampNanos))/1_000_000)
                .put("reaction_time_basis",if(mode=="Live")"first_observed_capture_utc_to_dispatch" else "first_observed_frame_1x_execution_to_dispatch"))
        } catch(e: CancellationException){throw e}
        catch(e: Exception){
    ...
    private suspend fun applyRoute(route: RouteResultDto,token: Int) = withContext(Dispatchers.Main) {
        ...
        if(state.value.ttsReady) {
            ...
            val result=tts?.speak("${if(snapshot.revision>1)"우회 경로로 변경했습니다. " else "안내를 시작합니다. "}남은 거리 ${snapshot.distanceMeters.toInt()}미터입니다.",TextToSpeech.QUEUE_FLUSH,null,lastUtteranceId)
    ...
    private suspend fun finish(drainSpeech: Boolean=false) {
        ...
        writer=null
        state.update{it.copy(running=false,terminal=true,message=if(it.arrived)"목적지 도착 · 기록 저장 완료" else "기록 저장 완료 · 현장 수용 기준은 결과 로그로 별도 확인합니다.")}
    ...
    @Synchronized private fun writeSummary(status: String,error: String?) {
        ...
        val s=state.value
        File(runDirectory,"run_summary.json").writeText(JSONObject().put("status",status).put("run_id",runId).put("input_mode",mode).put("clip_id",clipId)
